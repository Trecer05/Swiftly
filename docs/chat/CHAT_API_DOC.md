...
#### GET /team/{team_id}/applications

**Описание:**
Возвращает список заявок пользователей на вступление в команду.
Доступно **только владельцу команды**.
...
**Возможные коды ответа:**
- `200` — Заявки успешно получены
- `400` — Некорректный team_id
- `401` — Пользователь не авторизован
- `403` — Пользователь не является владельцем команды
- `404` — Команда не найдена
- `500` — Внутренняя ошибка сервера

---

#### PUT /team/{team_id}/applications/{application_id}

**Описание:**
Обновляет статус заявки пользователя (принять / отклонить).
Доступно **только владельцу команды**.
...
- `400` — Некорректные параметры или JSON
- `401` — Неавторизован
- `403` — Доступ запрещён (не владелец)
- `404` — Команда или заявка не найдены
- `500` — Внутренняя ошибка сервера

#### POST /team/{team_id}/applications

*(Создание приглашения: join-code)*

**Описание:**
Создаёт приглашение (join code) для вступления в команду.
Доступно **только владельцу команды**.
...
```json
{
  "expires_at": "2025-12-01T18:00:00Z",    // опционально
  "is_single_use": true                    // опционально
}
```
...
**Возможные коды ответа:**
- `200` — Код успешно создан
- `400` — Некорректный JSON
- `401` — Неавторизован
- `403` — Доступ запрещён (не владелец команды)
- `404` — Команда не найдена
- `500` — Внутренняя ошибка сервера
...
**Описание:**
Добавляет пользователя в команду по приглашению (join code).
Используется, например, при переходе по ссылке:

```
POST /join/team/ABCD1234
```
...
**Возможные коды ответа:**
- `200` — Успешное вступление
- `401` — Пользователь не авторизован
- `403` — Срок действия кода истёк
- `404` — Код не найден
- `500` — Внутренняя ошибка сервера
